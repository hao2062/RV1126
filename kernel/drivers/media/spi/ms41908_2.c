...
#include "zoom_mapping_table.h" // 引入 zoom_mapping_table.h

/* 添加 PI_TEST 作为调试开关*/
#define PI_TEST
/* 添加 REINIT_BOOT 设置开机自启*/
#define REINIT_BOOT

#define DRIVER_VERSION	KERNEL_VERSION(0, 0x01, 0x00)

#define DRIVER_NAME "ms41908_2"
// #define DRIVER_NAME "focus,test"
...
struct motor_dev {
	...
	// SPI 设备文件相关
    int major;                     // 字符设备主设备号
    struct class *cls;            // sysfs class，用于 device_create
    struct device *dev;           // 创建设备节点对应的 device
    struct cdev cdev;  

};

static struct motor_dev *g_motor = NULL;
...
// 声明注册函数（由 ms41908_1 导出）
extern void register_zoom_sync_callback(void (*cb)(int));
...
// 控制电机运动的核心函数，涉及电机运动状态设置，步进计算，SPI 写命令，定时器管理。
// 电机运行状态设置函数：根据输入的 pos，控制电机运动

// 1. 函数形参：pos-目标位置 motor-电机结构体 ext_dev-镜头 is_need_update_tim：是否要更新时间 。。。
				    ...
				    bool is_need_reback)
{
	...
	struct run_data_s run_data = ext_dev->run_data;    // 电机控制相关，包含步数，运动模式等
	u32 micro = 0;      // 微步数
	...
	is_need_reback = false;	// 默认不需要回退

	// pr_info("Before start: move_status = %d, is_running = %d\n", ext_dev->move_status, ext_dev->is_running);
    // pr_info("=== RUN_DATA INPUT ===\n");
	// pr_info("ppw = 0x%04x, psum = %u, intct = 0x%04x, micro = %u\n",run_data.ppw, run_data.psum, run_data.intct, run_data.micro);
	...
	if (ext_dev->type == TYPE_FOCUS) {
			motor_name = "focus";
	} else if (ext_dev->type == TYPE_ZOOM) {
		    motor_name = "zoom2";
	} else if (ext_dev->type == TYPE_ZOOM1) {
		    motor_name = "zoom3";
	} 
	printk("=== DEBUG [%s motor] ===\n", motor_name);
	printk("Target pos        : %d\n", pos);
	printk("Current last_pos  : %d\n", ext_dev->last_pos);
	printk("Need reback?      : %d\n", is_need_reback);
	printk("Reback steps      : %d\n", ext_dev->reback);
	printk("Final move count  : %d\n", mv_cnt);
	...
	// 只打印 Zoom2 / Zoom3，且 mv_cnt 为 1700 或 2500
	// if ((ext_dev->type == TYPE_ZOOM || ext_dev->type == TYPE_ZOOM1) &&
	// 	(mv_cnt == 1700 || mv_cnt == 2500)) {

	// 	pr_info("[SPI][%s] Write reg=0x20, val=0x%04x", motor_name, 0x1a01);
	// 	pr_info("[SPI][%s] Write reg=0x%02x (ppw), val=0x%04x",
	// 			motor_name, ext_dev->reg_op->reg.ppw, run_data.ppw | (run_data.ppw << 8));
	// 	pr_info("[SPI][%s] Write reg=0x%02x (psum), val=0x%04x",
	// 			motor_name, ext_dev->reg_op->reg.psum, psum);
	// 	pr_info("[SPI][%s] Write reg=0x%02x (intct), val=0x%04x",
	// 			motor_name, ext_dev->reg_op->reg.intct, ext_dev->run_data.intct);
	// }
	
	
	// pr_info("[SPI CMD] reg=0x20 val=0x%04x | reg=0x%02x (ppw) val=0x%04x | reg=0x%02x (psum) val=0x%04x | reg=0x%02x (intct) val=0x%04x\n",
    //     0x1a01,
    //     ext_dev->reg_op->reg.ppw, run_data.ppw | (run_data.ppw << 8),
    //     ext_dev->reg_op->reg.psum, psum,
    //     ext_dev->reg_op->reg.intct, ext_dev->run_data.intct);

	// pr_info("[SET] run_data.count = %d, cur_count = %d, psum = %d, intct = 0x%x\n",
	// 	ext_dev->run_data.count,
	// 	ext_dev->run_data.cur_count,
	// 	ext_dev->run_data.psum,
	// 	ext_dev->run_data.intct);
...
static int motor_dev_parse_dt(struct motor_dev *motor)
{
	...
	if (motor->is_use_zoom) {
		...
		ret = of_property_read_s32(node,
					   "zoom-min-pos",
					   &motor->zoom->min_pos);
		pr_info("zoom-min-pos = %d", motor->zoom->min_pos);	// zoom-min-pos = 0
		if (ret != 0) {
...
static void motor_config_dev_next_status(struct motor_dev *motor, struct ext_dev *dev)
{
	...
	// pr_info("[NEXT_STATUS] cur_count = %d, ppw = 0x%x, intct = 0x%x\n",
...
static void motor_op_work(struct work_struct *work)
{
	...
	// pr_info("********** b ***********");
	// pr_info("=== motor->dev0->reback_status = %d ===\n ", motor->dev0->reback_status);
	// pr_info("=== motor->dev0->run_data.cur_count = %d ===\n", motor->dev0->run_data.cur_count);
	// pr_info("=== motor->dev0->is_need_reback = %d ===\n", motor->dev0->is_need_reback);
	// pr_info("=== motor->dev0->cur_back_delay = %d ===\n", motor->dev0->cur_back_delay);
	// pr_info("=== motor->dev0->max_back_delay = %d ===\n", motor->dev0->max_back_delay);
	if (motor->dev0 && motor->dev0->run_data.cur_count == 0 &&
	   motor->dev0->is_need_reback) {
		if (motor->dev0->cur_back_delay < motor->dev0->max_back_delay) {
			...
			printk("=== motor_op_work: dev0 reback ===\n");
	...
	// printk(" motor->dev0->move_status = %d", motor->dev0->move_status);	
...
static int motor_g_volatile_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_FOCUS_ABSOLUTE:
		printk("=== motor_g_volatile_ctrl: V4L2_CID_FOCUS_ABSOLUTE ===\n");
...
static int motor_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	// 	int max_pos = 2505;
	// 	int steps = 64;
	// 	int mapped_val = min_pos + (ctrl->val * (max_pos - min_pos) / steps);
		
	// 	printk("=== step ori = %d, step used = %d\n", ctrl->val, mapped_val);
	...
	// 	wait_for_motor_stop(motor, motor->focus);
	// 	dev_dbg(&motor->spi->dev, "set focus pos %d (mapped from %d)\n", mapped_val, ctrl->val);
	// 	break;
	// }
...
static int motor_set_zoom_follow(struct motor_dev *motor, struct rk_cam_set_zoom *mv_param)
{
	...
	mv_param->is_need_focus_reback = false;

	dev_info(&motor->spi->dev, "motor_set_zoom_follow: setzoom_cnt = %d, zoom_reback = %d, focus_reback = %d\n",
		mv_param->setzoom_cnt,
		mv_param->is_need_zoom_reback,
		mv_param->is_need_focus_reback);

	for (j = 0; j < mv_param->setzoom_cnt; j++) {
		dev_info(&motor->spi->dev, "zoom_pos[%d]: zoom = %d, focus = %d\n",
			j,
			mv_param->zoom_pos[j].zoom_pos,
			mv_param->zoom_pos[j].focus_pos);
	}
			 ...
			 int min, int max)
{
	...
	printk("=== Function: motor_find_pi_binarysearch() ===\n");
		     ...
		     struct ext_dev *ext_dev, int step)
{
	...
	for (i = ext_dev->last_pos + step; i < idx_max; i += step) {
		...
		// 获取当前的光耦状态
		gpio_val = gpiod_get_value(ext_dev->pic_gpio);
		pr_info("=== motor %d : gpio_val = %d ===", ext_dev->type, gpio_val);
		// 如果当前值和初始值不同，短暂延时确认
		if (tmp_val != gpio_val) {
			usleep_range(10, 20);
			gpio_val = gpiod_get_value(ext_dev->pic_gpio);
		...
		if (tmp_val != gpio_val) {
			...
			pr_info("Find pi, min = %d, max = %d, is_find_pi = %d", min, max, is_find_pi);
...
static int motor_reinit_focus(struct motor_dev *motor)
{
	...
	printk("=== Function: motor_reinit_focus ===");

	if (has_run) {
		pr_info("motor_reinit_focus already called once, skipping.\n");
		return 0;
	}
	has_run = true;
	...
	if (!IS_ERR(motor->focus->pic_gpio)) {
		...
		if (motor->pi_gpio_usecnt == 0) {
			...
			if (!IS_ERR(motor->focus->pie_gpio))
				gpiod_set_value(motor->focus->pie_gpio, 1);    // 原来是 0
			msleep(250);
		}
		...
		#endif
		ret = motor_find_pi(motor, motor->focus, 200);    // 200
		if (ret < 0) {
			dev_info(&motor->spi->dev,
...
{
	printk("=== Function: motor_reinit_focus_pos ===");
	if (!motor->focus) {
		dev_err(&motor->spi->dev,
			"not support focus\n");
		return;
...
static int  motor_reinit_zoom(struct motor_dev *motor)
{
	...
	if (has_run) {
		pr_info("motor_reinit_zoom already called once, skipping.\n");
		return 0;
	}
	has_run = true;
	...
	if (!IS_ERR(motor->zoom->pic_gpio)) {
		...
		if (motor->pi_gpio_usecnt == 0) {
			...
			// 删除：删掉对 pia 管脚的操作
			if (!IS_ERR(motor->zoom->pia_gpio))
				gpiod_set_value(motor->zoom->pia_gpio, 1);
			// 删除：删掉对 光圈电机的操作
			if (!IS_ERR(motor->zoom->pie_gpio))
		...
		pr_info("min = %d, max = %d, ret = %d", motor->zoom->min_pos, motor->zoom->max_pos, ret);
		#endif
...
static int motor_reinit_zoom1(struct motor_dev *motor)
{
	...
	if (!IS_ERR(motor->zoom1->pic_gpio)) {
		...
		// pr_info("motor->zoom3->last_pos = %u", motor->zoom1->last_pos);		
		ret = set_motor_running_status(motor,
					       motor->zoom1,
					       0,
					       false,
		...
		#endif
		ret = motor_find_pi(motor, motor->zoom1, 200);		// 200 -> 1
		if (ret < 0) {
			dev_err(&motor->spi->dev,
		...
		// pr_info("motor->zoom1->min_pos = %d, motor->zoom1->max_pos = %d",
		// motor->zoom1->min_pos, motor->zoom1->max_pos);
...
//#define REBACK_CTRL_BY_DRV
	static int motor_set_focus(struct motor_dev *motor, struct rk_cam_set_focus *mv_param)
	{
		...
		printk("=== Function: motor_set_focus ===");
...
static long motor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RK_VIDIOC_VCM_TIMEINFO:
		// printk("=== Function: RK_VIDIOC_VCM_TIMEINFO ===");
		mv_tim = (struct rk_cam_vcm_tim *)arg;
	...
	case RK_VIDIOC_ZOOM_TIMEINFO:
		// printk("=== Function: RK_VIDIOC_ZOOM_TIMEINFO ===");
		mv_tim = (struct rk_cam_vcm_tim *)arg;
	...
	case RK_VIDIOC_FOCUS_SET_BACKLASH:
		printk("=== Function: RK_VIDIOC_FOCUS_SET_BACKLASH ===");
		pbacklash = (u32 *)arg;
		motor->focus->backlash = *pbacklash;
		break;
	case RK_VIDIOC_ZOOM_SET_BACKLASH:
		printk("=== Function: RK_VIDIOC_ZOOM_SET_BACKLASH ===");
		pbacklash = (u32 *)arg;
	...
	case RK_VIDIOC_ZOOM_SET_POSITION:					// 设置变焦电机位置
		// printk("=== Function: RK_VIDIOC_ZOOM_SET_POSITION ===");		
		mv_param = (struct rk_cam_set_zoom *)arg;
		ret = motor_set_zoom_follow(motor, mv_param);
		break;
	case RK_VIDIOC_FOCUS_SET_POSITION:					// 设置对焦电机位置
		printk("=== Function: RK_VIDIOC_FOCUS_SET_POSITION ===");	
...
static long motor_compat_ioctl32(struct v4l2_subdev *sd, unsigned int cmd, unsigned long arg)
{
	...
	// printk("=== Function: motor_compat_ioctl32 ===");
...
static void dev_param_init(struct motor_dev *motor)
{
	...
	if (motor->is_use_zoom1) {
		...
		if (motor->zoom1->reback != 0) {
			...
			if (motor->zoom1->is_half_step_mode)							// 选择半步
				step = mv_cnt * 4;
			else
				step = mv_cnt * 8;
...
static long motor_ioctl_spi(struct file *file, unsigned int cmd, unsigned long arg)
{
    int pos = (int)arg;

	struct motor_dev *motor = g_motor;

    pr_info("[motor_ioctl] cmd=0x%x, pos=%d\n", cmd, pos);

    switch (cmd) {
    case MOTOR_IOC_ZOOM:
        pr_info("[motor_ioctl] MOTOR_IOC_ZOOM cmd received");
        set_motor_running_status(motor, motor->zoom, pos, true, false, false);
        wait_for_motor_stop(motor, motor->zoom);
        break;

    case MOTOR_IOC_ZOOM1:
        pr_info("[motor_ioctl] MOTOR_IOC_ZOOM1 cmd received");
        set_motor_running_status(motor, motor->zoom1, pos, true, false, false);
        wait_for_motor_stop(motor, motor->zoom1);
        break;
...
void zoom_sync_handler(int zoom1_pos)
{
	...
	printk("[ZoomSync] zoom1_pos=%d\n", zoom1_pos);

    idx = 731 - zoom1_pos;

	target_zoom2 = zoom_map[idx].zoom2;
    target_zoom3 = zoom_map[idx].zoom3;

    // if (idx < 0 || idx >= zoom_map_size) {
    //     pr_warn("[ZoomSync] Invalid Zoom1 pos %d → idx %d out of bounds!\n", zoom1_pos, idx);
    //     return;
    // }

    // pr_info("[ZoomSync] Zoom1=%d → Zoom2=%d, Zoom3=%d\n",
    //         zoom1_pos, target_zoom2, target_zoom3);

    set_motor_running_status(motor, motor->zoom, target_zoom2, true, false, false);
    wait_for_motor_stop(motor, motor->zoom);

    set_motor_running_status(motor, motor->zoom1, target_zoom3, true, false, false);
    wait_for_motor_stop(motor, motor->zoom1);
}

static int motor_open(struct inode *inode, struct file *file)
{
    struct motor_dev *motor = container_of(inode->i_cdev, struct motor_dev, cdev);
    file->private_data = motor;
    pr_info("[motor_open] device opened\n");
    return 0;
}
...
static const struct file_operations motor_fops = {
    .owner = THIS_MODULE,
    .open = motor_open,
    .unlocked_ioctl = motor_ioctl_spi,
};
...
static int motor_dev_probe(struct spi_device *spi)
{
	...
	printk("=== Function: motor_dev_probe ===\n");
	...
	pr_info("into motor_initialize_controls");
	...
	pr_info("end motor_initialize_controls");
	// 根据设备朝向设置子设备名称
	...
	// 2. 定义设备号、class、device 指针（定义在 motor_dev 结构里比较好）
	motor->major = register_chrdev(0, "ms41908_2", &motor_fops);	

	motor->cls = class_create(THIS_MODULE, "ms41908_2_class");

	motor->dev = device_create(motor->cls, NULL, MKDEV(motor->major, 0), NULL, "ms41908_2");
	
	g_motor = motor;  // 保存全局变量

	pr_info("=== register_zoom_sync_callback ===");
	register_zoom_sync_callback(zoom_sync_handler);  // 注册回调
...
static int motor_dev_remove(struct spi_device *spi)
{
	...
	device_destroy(motor->cls, MKDEV(motor->major, 0));
	class_destroy(motor->cls);
	unregister_chrdev(motor->major, "ms41908_2");
...
static const struct of_device_id motor_dev_of_match[] = {
	...
	{.compatible = "ms41908_2,zoom2_zoom3", },
	{},
};
#endif
