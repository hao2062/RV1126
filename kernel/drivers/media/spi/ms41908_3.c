...
extern void register_iris_sync_callback(void (*cb)(int choose, int iris_pos));
				    ...
				    bool is_need_reback)
{
	...
	pr_info("[SPI CMD] reg=0x%02x (intct) val=0x%04x\n",
...
static int motor_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_IRIS_ABSOLUTE:
		...
		break;
	case V4L2_CID_FOCUS_ABSOLUTE:
		pr_info("=== motor_s_ctrl: V4L2_CID_FOCUS_ABSOLUTE ===\n");
		ret = set_motor_running_status(motor,
					       motor->focus,
					       ctrl->val,
					       true,
	...
	case V4L2_CID_ZOOM_ABSOLUTE:
		pr_info("=== motor_s_ctrl: V4L2_CID_ZOOM_ABSOLUTE ===\n");
		ret = set_motor_running_status(motor,
					       motor->zoom,
					       ctrl->val,
					       true,
					       false,
					       false);
		// pr_info("=== set zoom1 and zoom2-3 ===\n");
		wait_for_motor_stop(motor, motor->zoom);
		// 调用 Zoom2 + 3
		pr_info("=== set zoom1 and zoom2-3 ===\n");
		// if (sync_zoom_callback)
    		// sync_zoom_callback(ctrl->val);  // 替换 zoom1_pos 为实际变量名
...
static int motor_set_zoom_follow(struct motor_dev *motor, struct rk_cam_set_zoom *mv_param)
{
	...
	for (i = 0; i < mv_param->setzoom_cnt; i++) {
		...
		pr_info("=== set zoom1 and zoom2-3 ===\n");
		// if (sync_zoom_callback)
    		// sync_zoom_callback(mv_param->zoom_pos[i].zoom_pos);  // 替换 zoom1_pos 为实际变量名
...
static int motor_reinit_piris(struct motor_dev *motor)
{
	...
	if (!IS_ERR(motor->piris->pic_gpio)) {
		...
		ret = motor_find_pi(motor, motor->piris, 5);
		if (ret < 0) {
			dev_err(&motor->spi->dev,
		...
		motor->piris->max_pos = 44;
		pr_info("piris min_pos = %d, max_pos = %d", motor->piris->min_pos, motor->piris->max_pos);
...
static long motor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RK_VIDIOC_VCM_TIMEINFO:
		...
		if (motor->focus->is_mv_tim_update) {
			...
			// printk("get_focus_move_tim 0x%lx, 0x%lx, 0x%lx, 0x%lx\n",
			// 		mv_tim->vcm_start_t.tv_sec,
			// 		mv_tim->vcm_start_t.tv_usec,
			// 		mv_tim->vcm_end_t.tv_sec,
			// 		mv_tim->vcm_end_t.tv_usec);
...
static void dev_param_init(struct motor_dev *motor)
{
	...
	if (motor->is_use_p_iris) {
		...
		motor->piris->run_data.psum = motor->vd_fz_period_us *
					      motor->piris->start_up_speed * 4 / 1000000;
		pr_info("vd_fz_period_us %u, psum %u, start_up_speed %u\n",
			motor->vd_fz_period_us,
...
static long motor_ioctl_spi(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
    case MOTOR_IOC_PIRIS:
        pr_info("[motor_ioctl] MOTOR_IOC_ZOOM cmd received");
        set_motor_running_status(motor, motor->piris, pos, true, false, false);
        wait_for_motor_stop(motor, motor->piris);
        break;
...
void iris_sync_handler(int choose, int iris_pos)
{
	...
	case 0:	// 选择光圈
		set_motor_running_status(motor, motor->piris, iris_pos, true, false, false);
    	wait_for_motor_stop(motor, motor->piris);
	// case 1:
	// 	if (!motor->piris->is_mv_tim_update)
	// 		usleep_range(motor->piris->move_time_us,
...
void ms41908_get_piris_timeinfo(struct rk_cam_vcm_tim *out_tim) {
	...
	pr_info("=== Function: ms41908_get_piris_timeinfo ===");

	if (!motor->piris->is_mv_tim_update)
		usleep_range(motor->piris->move_time_us,
				motor->piris->move_time_us + 1000);
	if (motor->piris->is_mv_tim_update) {
		memcpy(out_tim, &motor->piris->mv_tim, sizeof(*out_tim));
...
static int motor_dev_probe(struct spi_device *spi)
{
	...
	motor->major = register_chrdev(0, "ms41908_3", &motor_fops);	

	motor->cls = class_create(THIS_MODULE, "ms41908_3_class");

	motor->dev = device_create(motor->cls, NULL, MKDEV(motor->major, 0), NULL, "ms41908_3");
	
	g_motor = motor;  // 保存全局变量

	pr_info("=== register_iris_sync_callback ===");
	register_iris_sync_callback(iris_sync_handler);  // 注册回调
...
static int motor_dev_remove(struct spi_device *spi)
{
	...
	unregister_chrdev(motor->major, "ms41908_3");
