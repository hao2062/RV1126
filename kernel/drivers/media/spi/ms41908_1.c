...
// === Zoom1 -> Zoom2/3回调函数
static void (*sync_zoom_callback)(int zoom1_pos) = NULL;

void register_zoom_sync_callback(void (*cb)(int))
{
    sync_zoom_callback = cb;
}
EXPORT_SYMBOL(register_zoom_sync_callback);  // 注册为全局函数！

// === Iris -> Iris Set Pos 回调函数
static void (*sync_iris_callback)(int choose, int iris_pos) = NULL;

void register_iris_sync_callback(void (*cb)(int choose, int iris_pos))
{
    sync_iris_callback = cb;
}
EXPORT_SYMBOL(register_iris_sync_callback);  // 注意加这个！
				    ...
				    bool is_need_reback)
{
	...
	printk ("=== Function: set_motor_running_status. ===");
	printk ("step = %d ", pos);
	...
	if (ext_dev->type == TYPE_FOCUS) {
		    ...
		    motor_name = "zoom";
	} else if (ext_dev->type == TYPE_IRIS) {
		    motor_name = "iris";
	} 
...
static int motor_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	printk("=== bbb ===\n");
			...
			if (sync_iris_callback)
    			sync_iris_callback(0, ctrl->val-4);  // 替换 zoom1_pos 为实际变量名

			// dev_dbg(&motor->spi->dev, "set piris pos %d\n", ctrl->val);
		}
		break;
	case V4L2_CID_FOCUS_ABSOLUTE:
		printk("=== motor_s_ctrl: V4L2_CID_FOCUS_ABSOLUTE ===\n");
		ret = set_motor_running_status(motor,
					       motor->focus,
					       ctrl->val,
					       true,
...
static int motor_reinit_focus(struct motor_dev *motor)
{
	...
	if (!IS_ERR(motor->focus->pic_gpio)) {
		...
		min = motor->focus->min_pos;
		motor->focus->min_pos = -min;
		pr_info("min = %d, max = %d, ret = %d", motor->focus->min_pos, motor->focus->max_pos, ret);
		#endif
...
static long motor_ioctl(struct v4l2_subdev *sd, unsigned int cmd, void *arg)
{
	...
	case RK_VIDIOC_IRIS_TIMEINFO:
		...
		ms41908_get_piris_timeinfo(mv_tim);	

		// 原版 RK_VIDIOC_IRIS_TIMEINFO
		// mv_tim = (struct rk_cam_vcm_tim *)arg;
...
static const struct of_device_id motor_dev_of_match[] = {
	...
	{.compatible = "ms41908_1,focus_zoom1", },
	{},
};
#endif
